...
utils.externalDmgReducs = {
	...
	--Wisp: Overcharge
	{
		...
		type = 1,
		sourceTeam = 1,
		sourseHero = CDOTA_Unit_Hero_Wisp,
	...
	--Treant Protector: Living Armor
	{
		...
		type = 0,
		sourceTeam = 1,
		sourseHero = CDOTA_Unit_Hero_Treant,
...
utils.externalDmgAmps = {
	--Shadow Demon: Soul Catcher
	{
		...
		sourceTeam = -1,
		amp = {.2,.3,.4,.5},
		sourseHero = CDOTA_Unit_Hero_Shadow_Demon,
	...
	--Chen: Penitence
	{
		...
		sourceTeam = -1,
		amp = {.08,.16,.24,.32},
		sourseHero = CDOTA_Unit_Hero_Chen,
	...
	if dmgType ~= DAMAGE_HPRM then
		...
		if self:DoesHaveModifier("modifier_medusa_mana_shield") then
			...
			if spell then
				--Calculate
				local reduce = 0	
				if self.mana >= tempDmg*.5/(spell:GetSpecialData("damage_per_mana")) then
					reduce = .5
				else
					reduce = self.mana*(spell:GetSpecialData("damage_per_mana"))/tempDmg
				end
				tempDmg = tempDmg * (1 - reduce)
			end
		end		
